...
export async function POST(request: NextRequest, { params }: { params: Promise<{ brandId: string }> }) {
  try {
    ...
    if (readError || !existing?.id) throw new ApiError(404, "ALPI-BRD-NOTF-419", "Brand not found.");
    if (existing.tenant_id !== tenant.id) throw new ApiError(403, "ALPI-BRD-PERM-420", "Cannot activate brand from another tenant.");

    if (existing.is_active) return NextResponse.json({ ok: true, item: { id: existing.id, is_active: true } });
    ...
    if (error || !data) throw new ApiError(500, "ALPI-BRD-INT-421", error?.message || "Failed to activate brand.");
